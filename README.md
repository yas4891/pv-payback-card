...
- Optional payback duration in years, months, and days, measured from today or the accounting start date.
...
These options cover the normal production-based setup, pricing, card identity, and the primary display style.
...
These options provide a direct self-consumption input, detailed visibility controls, location-based seasonality, and annual discounting.
...
| `self_consumption_baseline`       | No       | `0`        | Self-consumption counter reading at `start_date`, in `kWh`. Negative values can carry energy forward after a counter replacement. Example: `-20000`.                                                                                                                      |
...
| `payback_date_format`             | No       | `absolute` | Use `absolute` for a calendar date or `relative` for a duration. Example: `relative`.                                                                                                                                                                                     |
| `payback_date_relative_reference` | No       | `now`      | For relative durations, use `now` to show remaining time or `start_date` to show total time since the accounting start. Example: `start_date`.                                                                                                                            |
